...
**Status:** REVISED — Backoffice Admin Dashboard Live, 45/46 Tech Issues Closed
...
21. **Post-Launch Cross-Pod Security Audit.** All 38 agents consulted internally within their pods. Safety Guardian fail-open bug identified as most critical finding — defaults to SAFE on LLM parse failure, bypassing safety on potentially dangerous messages. 20 new issues (#74-#93) created and assigned.
22. **Sprint 5-7 Post-Launch Roadmap.** Sprint 5: Security hardening (auth, CORS, input validation, fail-closed safety) + legal foundation (incorporation, ToS, Privacy Policy). Sprint 6: Agent build-out (Emergency Response, Phase-Crisis, CPsychO meta-audit) + go-to-market (waitlist, clinical co-founder). Sprint 7: Investor materials + branch protection + mobile UX.
23. **Safety Guardian Fail-Closed Doctrine.** Established principle: ALL safety-critical agents must fail-closed (halt=true) on ANY parse/network failure. False positives are acceptable; false negatives are not. Applied to Safety Guardian, Orchestrator, and Emergency Response Agent.
24. **Regex Pre-Screen Safety Layer.** New deterministic safety layer before LLM calls — fast keyword matching provides guaranteed safety floor independent of LLM availability, latency, or correctness.
25. **Legal Foundation Sprint.** Delaware C-Corp incorporation, ToS v1.0 with medical disclaimer + duty-to-warn, GDPR-compliant Privacy Policy v1.0 — all blocked as P0 before first real user.
26. **Customer Discovery Sprint.** 50 problem interviews mandated before product decisions. 6-question script targeting dating, cohabitating, and married segments across 6 channels.

### Changes from v1.6.0

27. **Azure OpenAI Production Deployment.** Provisioned `relio-openai` in Sweden Central with GPT-4.1 (`gpt-41`, GlobalStandard) and GPT-4.1-mini (`gpt-41-mini`, GlobalStandard). All 5 pipeline agents now run on Azure OpenAI via managed identity — zero API keys, full EU data residency.
28. **Security Hardening Sprint Complete.** Server hardened with Helmet security headers, express-rate-limit (30 req/min), Zod input validation (UUID format, 2000 char max), CORS lockdown, 10kb body limit. JWT auth middleware on REST + WebSocket. Safety Guardian fail-closed on parse errors.
29. **LLM Gateway Circuit Breaker + Token Budgets.** Circuit breaker (3 failures → 30s open) prevents cascade failures. Per-user daily token budget (50K default) prevents abuse. `DefaultAzureCredential` for managed identity auth with automatic token caching.
30. **Database Architecture Finalized.** Tier 1 PostgreSQL schema with Row-Level Security (users, tier1_messages, safety_audit_log, journal_entries, 90-day auto-purge). Tier 3 PostgreSQL schema (rooms, room_members, tier3_messages, sessions, invite codes). No foreign keys between databases per `architect-dual-context-db` skill mandate.
31. **CI/CD Pipeline Maturity.** Three workflows: `backend-ci.yml` (typecheck + tests on PR), `security-scan.yml` (CodeQL + npm audit + gitleaks), `backend-cd.yml` (OIDC → ACR → Container Apps → smoke test). Branch protection on `main` enforces PR + review + CI pass.
32. **Infrastructure Hardened.** Terraform remote state backend (Azure Blob), ACR admin disabled (managed identity pull), Cosmos DB private endpoint, NSGs on postgres + private-endpoints subnets, Key Vault RBAC with purge protection, Container App managed identity with `Cognitive Services OpenAI User` + `Key Vault Secrets User` roles.
33. **Mobile-Backend Connectivity.** API client rewritten with auth token support (Bearer REST, ?token= WS), fixed syntax errors, port alignment, async WebSocket connect, 401/4401 error handling. ContextBanner component for private/shared visual demarcation.
34. **Cost Optimization.** Scale-to-zero enabled (min=0, max=10 replicas), $50/month budget alert with email notifications at 80%/100% thresholds.

### Changes from v1.7.0

35. **Backoffice Admin Dashboard.** 10 admin API endpoints under `/api/v1/admin/*` with k-anonymity enforcement (groups < 5 suppressed), admin JWT role requirement, audit logging. Pipeline telemetry auto-records latency, safety events, and token usage. 7-page React frontend (Dashboard, Users, Phases, Subscriptions, Pipeline, Safety, Feedback) with Relio design language.
36. **User Feedback Collection System.** Four feedback types: post-session ratings (1-5 stars), weekly pulse (3 questions), NPS (0-10), churn interviews. PII-redacted comments, anonymized userId in admin view. NPS score auto-calculated.
37. **6 New Agent Skills Created.** `build-admin-api` (backend-developer), `design-admin-dashboard` (ui-ux-expert), `build-kql-workbooks` (cloud-architect), `test-admin-privacy` (fullstack-qa), `design-subscription-analytics` (CRO/CFO), `collect-user-feedback` (CPO/mobile-dev). Skills-Builder executed EvoSkill Loop for admin domain.
38. **Privacy-by-Design Admin Architecture.** Admin API connects ONLY to Tier 3 shared database — physically cannot query Tier 1 private data or Tier 2 clinical profiles. No export/download buttons. 100-record pagination limit. Admin audit trail.
...
- **Primary Region:** Azure Sweden Central (`swedencentral`) — EU data residency compliant
