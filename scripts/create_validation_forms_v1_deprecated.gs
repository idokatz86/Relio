/**
 * Relio Market Validation — Google Forms Auto-Generator
 * 
 * HOW TO USE:
...
function createEnglishForm() {
  ...
  form.addParagraphTextItem()
    .setTitle('If the above didn\'t fully solve the problem — why not?')
    .setRequired(false);
  ...
  // Q12 — Email (waitlist)
  form.addTextItem()
    .setTitle('Would you like to be notified when we launch? (Optional)')
    .setHelpText('Leave your email if you\'d like early access. Completely optional — we\'ll never share it.')
    .setRequired(false);

  // Q13 — Open
  form.addParagraphTextItem()
    .setTitle('Anything else you\'d like to share about communication in your relationship?')
    .setRequired(false);
...
function createHebrewForm() {
  ...
  // ש6 — למה לא עבד
  form.addParagraphTextItem()
    .setTitle('אם הנ"ל לא פתר את הבעיה לגמרי — למה?')
    .setRequired(false);
  ...
  // ש12 — מייל (רשימת המתנה)
  form.addTextItem()
    .setTitle('רוצים לקבל הודעה כשנשיק? (אופציונלי)')
    .setHelpText('השאירו מייל אם אתם רוצים גישה מוקדמת. לגמרי אופציונלי — לעולם לא נשתף את המייל שלכם.')
    .setRequired(false);

  // ש13 — פתוח
  form.addParagraphTextItem()
    .setTitle('עוד משהו שתרצו לשתף על תקשורת בזוגיות שלכם?')
    .setRequired(false);
