...
Audit the data pipeline to ensure PII redaction (Azure AI Language + Presidio) occurs before data hits the LLM Gateway (Azure API Management). Verify that names and locations are replaced with tags like PERSON1.
...
Verify that the backend includes automated deletion protocols that routinely purge Tier 1 data when it is no longer legally or functionally required. Default: 90 days. Azure Functions timer triggers for cleanup.
